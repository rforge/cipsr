...
## Set paths to common home and CIPSR internal files
paths <- new.env() # Set an enviornment to store file paths
home = getwd() # Get the home directory
unlink(paste(home,"My Output",sep="/"),recursive=TRUE,force=TRUE) # Delete old output from CIPS-R
engine = paste(home,"Engine",sep="/") # Location of the internal CIPS-R files, relative to home directory
assign('home',home,envir=paths) # Assign home directory path to the path enviornment
assign('engine',engine,envir=paths) # Assign the internal CIPS-R path to the path enviornment 
...
suppressMessages(require(sp,lib.loc=engine))
suppressMessages(require(raster,lib.loc=engine))
suppressMessages(require(rgdal,lib.loc=engine)) 

## Now load soil and weather coverages
setwd(engine) # Navigate to the folder where raster files are stored
rasters <- new.env() # Set up a raster enviornment to call
assign('whc50',raster("whc50.img"),envir=rasters) # Water holding capacity from 0 - 50 cm (%)
assign('pptdd5',raster("pptdd5.img"),envir=rasters) # Growing season precipitation (mm)
setwd(home) # Return to the home directory
}
...
setwd(get('engine',envir=paths))
flags = read.csv("flags.csv") # Read the table into R
...
dyn.load("CIPSEDIT.dll",type="Fortran") # Prepare subroutine
...
dyn.unload("CIPSEDIT.dll") # Unload the prepare subroutine
...
dyn.load("ORGWQ.dll",type="Fortran") # Load the Organon wood quality subroutine
...
dyn.unload("ORGWQ.dll") # Unload the wood quality DLL
...
source("treatments.R") # Source the treatment algorithms
...
as.integer(VERSION),as.integer(SPECIES),as.single(SITE_1),as.single(SITE_2),as.single(PDEN),as.single(DBH),as.single(HT),as.single(CR),as.single(SCR),as.single(EXPAN),as.single(MGEXP), 
as.single(DGRO), as.single(HGRO), as.single(CRCHNG), as.single(SCRCHNG),as.integer(BRCNT), as.integer(BRDIA), as.integer(BRHT), as.integer(JCORE), as.integer(IDIB) )
dyn.unload("ORGWQ.dll") # Unload the Organon wood quality subroutine
...
dyn.load("CIPSRUN.dll", type="Fortran") # Load the execute subroutine 
...
dyn.unload("CIPSRUN.dll") # Unload the execute subroutine 
...
if(unit$woodqual==1) {
## Call the wood quality subroutine in ORGWQ.dll to estimate wood quality
dyn.load("ORGWQ.dll",type="Fortran") # Load the wood quality DLL
glasslog = .Fortran("woodqual",as.integer(IJCALC),as.integer(IEVEN),as.integer(IFINAL),as.integer(ACTION),as.integer(BHAGE),as.integer(STAGE),as.integer(NINGRO),as.integer(NPTS),as.integer(NTREES),as.integer(NWQT), 
as.integer(VERSION),as.integer(SPECIES),as.single(SITE_1),as.single(SITE_2),as.single(PDEN),as.single(DBH),as.single(HT),as.single(CR),as.single(SCR),as.single(EXPAN),as.single(MGEXP), 
as.single(DGRO), as.single(HGRO), as.single(CRCHNG), as.single(SCRCHNG),as.integer(BRCNT), as.integer(BRDIA), as.integer(BRHT), as.integer(JCORE), as.integer(IDIB) )
dyn.unload("ORGWQ.dll") # Unload the wood quality subroutine
...
dyn.load("CIPSVOL.dll",type="Fortran") # Load the Organon volume subroutine
...
dyn.unload("CIPSVOL.dll") # Unload the volume calculation DLL
...
setwd(get('home',envir=paths)) # Return to the home directory
...
dyn.load("ORGEDIT.dll",type="Fortran") # Prepare subroutine
prepared = .Fortran("prepare",as.integer(VERSION),as.integer(NPTS),as.integer(NTREES),as.integer(STAGE),
as.integer(BHAGE),as.integer(SPECIES),as.integer(USER),as.integer(IEVEN),as.single(DBH),
as.single(HT), as.single(CR),as.single(EXPAN),as.single(RADGRO),as.single(RVARS),as.integer(SERROR), 
as.integer(TERROR), as.integer(SWARNING),as.integer(TWARNING),as.integer(IERROR),as.integer(IRAD),
as.single(GROWTH),as.single(ACALIB)) 
dyn.unload("ORGEDIT.dll") # Unload the prepare subroutine
...
dyn.load("ORGRUN.dll",type="Fortran") # Load the execute subroutine 
grow = .Fortran("execute",as.integer(CYCLG), as.integer(VERSION),as.integer(NPTS), as.integer(NTREES1), as.integer(STAGE), as.integer(BHAGE), 
as.integer(TREENO), as.integer(PTNO), as.integer(SPECIES), as.integer(USER),as.integer(INDS), as.single(DBH1), as.single(HT1), as.single(CR1), 
as.single(SCR1), as.single(EXPAN1), as.single(MGEXP),as.single(RVARS), as.single(ACALIB), as.single(PN), as.single(YSF),as.single(BABT), 
as.single(BART), as.single(YST), as.integer(NPR),as.integer(PRAGE), as.single(PRLH), as.single(PRDBH), as.single(PRHT), as.single(PRCR), 
...
as.integer(SWARNING), as.integer(TWARNING), as.integer(IERROR),as.single(DGRO), as.single(HGRO), as.single(CRCHNG), as.single(SCRCHNG), 
as.single(MORTEXP), as.integer(NTREES2), as.single(DBH2), as.single(HT2),as.single(CR2), as.single(SCR2), as.single(EXPAN2), as.single(STOR)) 
dyn.unload("ORGRUN.dll") # Unload the execute subroutine 
...
setwd(home) # Return to the users home directory; in case already not there!
...
.load.data <-
function( InputFile ) {
## Load the XL Connect package from the CIPS R engine folder; accounting for dependencies
suppressMessages(require("rJava",lib.loc=paste(getwd(),"Engine",sep="/")))
suppressMessages(require("XLConnect",lib.loc=paste(getwd(),"Engine",sep="/")))
...
## Ensure that the user-specified database is in the correct format
...
InputList = list(samples=readWorksheetFromFile(InputFile, sheet="samples"), units=readWorksheetFromFile(InputFile, sheet="units"),
