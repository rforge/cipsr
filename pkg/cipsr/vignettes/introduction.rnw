...
cipsrexam$units$variant = ifelse(cipsrexam$units$unit %in% c(2,13),1,3)  
...
# Use the traditional site index 
...
Now that \texttt{cipsrexam} has been modified for the goals of our analysis, we can call the \texttt{grow} function of \textit{cipsr}.  With the output from \texttt{grow} we can calculate the final yield of each unit and use that information in a simple linear model.
...
As indicated in the commented code, we find some evidence that site-index influenced the volume yield among the units.  We could now explore our analysis question in much greater detail using the saved output \texttt{grown} in the above example and with the analysis tools offered in R.  
...
\begin{verbatim}
# Obtain simulation data from one example unit
dat = lapply(cipsrexam,function(x) subset(x,unit=="A"))
dat$units$wantplot=0; dat$units$wanttable=0 

# Use the single unit to produce 100 (e.g. could be 10,000) units of data
...
# Example 1. Extract harvest yield in chunks: keep only the necessary data
grown = lapply(dat,function(x){
	out = grow(x)$samplelist
...
grown = lapply(dat,function(x){
		...
		# Condense data to just the final period of growth
...
grown = do.call(Map,c(rbind,grown)) # Make the list-of-lists one list
\end{verbatim}
