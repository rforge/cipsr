...
\begin{description}
	...
	\item[species] \hfill \\  
	Tree species identification code.  The table given below indicates the species name corresponding to its code and whether it is supported in the a model.  SWO is Southwestern Oregon, NWO is Northwest Oregon and SMC is the Stand Management Cooperative variants. \\
